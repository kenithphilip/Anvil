...
//
//   - Status FULFILLED / CANCELLED -> mark_complete.
//   - promised_date < now() and no shipment -> escalate (slipped).
//   - promised_date - now() within `eta_check_days_before` -> send_email.
...
export const deliveryEtaCheck = async (goal, ctx) => {
  ...
  const r = await svc.from("source_pos")
    .select("id, status, reference, supplier, supplier_contact_email, promised_date, ack_received_at")
  ...
  if (!po.promised_date) {
    return { thought: "PO has no promised_date; nothing to chase.", action: "noop", action_payload: { sleep_hours: 24 } };
  }
  const now = Date.now();
  const promisedMs = new Date(po.promised_date).getTime();
  ...
  const body = [
    ...
    "",
    "PO " + po.reference + " is promised for " + po.promised_date + " (" + daysLeft + " day" + (daysLeft === 1 ? "" : "s") + " from today).",
    "",
  ...
  return {
    thought: "Sending ETA check to " + po.supplier_contact_email + " (" + daysLeft + "d to promised date)",
    action: "send_email",
    action_payload: {
