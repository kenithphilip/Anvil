...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === "GET") {
      ...
      return json(res, 200, { field_map: s?.eclipse_field_map || {} });
    }
    if (req.method === "PUT" || req.method === "POST") {
      ...
      const u = await updateTenantSettings(svc, ctx.tenantId, { eclipse_field_map: map });
      await recordAudit(ctx, { action: "eclipse_field_map_updated", objectType: "tenant_settings", objectId: ctx.tenantId, detail: "entries=" + Object.keys(map).length });
      return json(res, 200, { ok: true, field_map: u?.eclipse_field_map || {} });
    }
