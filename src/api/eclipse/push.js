...
export default async function handler(req, res) {
  ...
  try {
    ...
    const idem = await checkExportIdempotency(svc, {
      tenantId: ctx.tenantId, orderId: orderQ.data.id, connector: "eclipse",
      payloadHash, allowReexport: body.reexport === true,
    });
    if (idem.idempotent) return json(res, 200, { ok: true, idempotent: true, external_id: idem.external_id, eclipse_id: idem.external_id });
    ...
    await recordExport(svc, { tenantId: ctx.tenantId, orderId: orderQ.data.id, connector: "eclipse", payloadHash, externalId });
