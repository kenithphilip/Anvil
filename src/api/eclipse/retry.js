...
const replayFor = (svc, settings, ctx) => async (row) => {
  let resp;
  try { resp = await eclipseFetch(settings, { method: "POST", path: "/eterm/orders", body: row.payload }); }
      ...
      if (orderQ.data) {
        const newResult = {
          ...
          external_systems: {
            ...
            eclipse: { id: externalId, external_id: externalId, status: "exported", last_attempt_at: new Date().toISOString() },
          },
        };
        await svc.from("orders").update({ result: newResult }).eq("id", orderQ.data.id);
      }
    }
    if (ctx) await recordAudit(ctx, { action: "eclipse_push", objectType: "order", objectId: row.order_id, detail: "retry_ok::id=" + externalId });
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (isCron) {
      const tenants = await svc.from("eclipse_retry_queue").select("tenant_id").eq("status", "pending")
      ...
      for (const tid of uniq) {
        ...
        const settings = eclipseDecryptCreds({ ...sRow.data, tenant_id: tid });
        if (!eclipseIsConfigured(settings)) continue;
        out.push({ tenant_id: tid, ...await drainRetryQueue(svc, "eclipse", { tenantId: tid, opts: {}, replay: replayFor(svc, settings, null), isRecoverable: httpIsRecoverable }) });
      }
    ...
    const settings = eclipseDecryptCreds({ ...sRow.data, tenant_id: ctx.tenantId });
    if (!eclipseIsConfigured(settings)) return json(res, 409, { error: { code: "ECLIPSE_NOT_CONFIGURED", message: "Eclipse not configured" } });
    const r = await drainRetryQueue(svc, "eclipse", {
