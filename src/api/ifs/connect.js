// POST /api/ifs/connect
//
// Stores IFS Cloud credentials on tenant_settings (encrypted when
...
import { ifsEncryptCreds, ifsDecryptCreds, ifsProbe } from "../_lib/ifs-client.js";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const enc = ifsEncryptCreds({ client_id: body.client_id, client_secret: body.client_secret });
    const updated = await updateTenantSettings(svc, ctx.tenantId, {
      ifs_base_url: String(body.base_url).replace(/\/+$/, ""),
      ifs_token_url: body.token_url,
      ifs_scope: body.scope || "openid profile INTEGRATION",
      ifs_company: body.company || null,
      ifs_projection: body.projection || "CustomerOrder.svc",
      ...enc,
    });
    const decrypted = ifsDecryptCreds({ ...updated, tenant_id: ctx.tenantId });

    let probe = null;
    let probeErr = null;
    try {
      probe = await ifsProbe(decrypted);
    } catch (err) {
      probeErr = err.message;
      probe = { ok: false, status: 0, body: { error: err.message } };
    }
    if (probe.ok) {
      await updateTenantSettings(svc, ctx.tenantId, { ifs_connected_at: new Date().toISOString() });
    }
