// POST /api/ifs/push  Body: { orderId, dry_run? }
//
// Pushes a single Anvil sales order into IFS Cloud as a Customer
// Order. Recoverable failures land in the retry queue; permanent
// failures return 502.
...
import { ifsDecryptCreds, ifsPushSalesOrder, ifsIsConfigured } from "../_lib/ifs-client.js";
...
const enqueueRetry = async (svc, tenantId, orderId, payload, status, err) => {
  await svc.from("ifs_retry_queue").insert({
    tenant_id: tenantId,
    order_id: orderId,
    payload,
    attempt_count: 1,
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!ifsIsConfigured(settings)) {
      return json(res, 409, { error: { code: "IFS_NOT_CONFIGURED", message: "IFS Cloud not configured" } });
    }
    ...
    // Idempotency guard: skip a re-export that already succeeded; block a
    // changed-payload re-export unless explicitly opted in.
    const payloadHash = orderPayloadHash(orderQ.data);
    const idem = await checkExportIdempotency(svc, {
      tenantId: ctx.tenantId, orderId: orderQ.data.id, connector: "ifs",
      payloadHash, allowReexport: body.reexport === true,
    });
    if (idem.idempotent) return json(res, 200, { ok: true, idempotent: true, external_id: idem.external_id, ifs_id: idem.external_id });
    if (idem.blocked) return json(res, idem.status, idem.body);

    let resp = null;
    try {
      resp = await ifsPushSalesOrder(settings, orderForPush, settings.ifs_field_map || {});
    } catch (err) {
    ...
    await recordAudit(ctx, {
      action: ok ? "ifs_push" : "ifs_push_failed",
      objectType: "order",
      objectId: orderQ.data.id,
      detail: ok ? ("ifs_id=" + externalId) : ("status=" + resp.status),
    });
    ...
    await recordExport(svc, { tenantId: ctx.tenantId, orderId: orderQ.data.id, connector: "ifs", payloadHash, externalId });
    return json(res, 200, { ok: true, ifs_id: externalId, status: resp.status });
  } catch (err) {
