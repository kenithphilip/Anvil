// /api/ifs/retry
// Drain the IFS Cloud retry queue with exponential backoff.

import { applyCors, handlePreflight, json, readBody, sendError } from "../_lib/cors.js";
import { resolveContext, requirePermission } from "../_lib/auth.js";
import { serviceClient } from "../_lib/supabase.js";
import { recordAudit } from "../_lib/audit.js";
import { ifsDecryptCreds, ifsIsConfigured, ifsPushSalesOrder } from "../_lib/ifs-client.js";
...
const replayFor = (svc, settings, ctx) => async (row) => {
  ...
  let resp;
  try {
    resp = await ifsPushSalesOrder(settings, { ...orderQ.data, customer }, settings.ifs_field_map || {});
  } catch (err) {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (isCron) {
      const tenants = await svc.from("ifs_retry_queue").select("tenant_id")
        .eq("status", "pending")
      ...
      for (const tid of uniq) {
        ...
        const settings = ifsDecryptCreds({ ...sRow.data, tenant_id: tid });
        if (!ifsIsConfigured(settings)) continue;
        out.push({
          tenant_id: tid,
          ...await drainRetryQueue(svc, "ifs", {
            tenantId: tid,
            opts: {},
