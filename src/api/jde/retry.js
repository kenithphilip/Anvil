// /api/jde/retry
// Drain the JDE retry queue with exponential backoff. Phase 5.4b cluster B.

import { applyCors, handlePreflight, json, readBody, sendError } from "../_lib/cors.js";
import { resolveContext, requirePermission } from "../_lib/auth.js";
import { serviceClient } from "../_lib/supabase.js";
import { recordAudit } from "../_lib/audit.js";
import { jdeDecryptCreds, jdeIsConfigured, jdePushSalesOrder } from "../_lib/jde-client.js";
...
const replayFor = (svc, settings, ctx) => async (row) => {
  ...
  let resp;
  try {
    resp = await jdePushSalesOrder(settings, { ...orderQ.data, customer }, settings.jde_field_map || {});
  } catch (err) {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (isCron) {
      const tenants = await svc.from("jde_retry_queue").select("tenant_id")
        .eq("status", "pending")
      ...
      for (const tid of uniq) {
        ...
        const settings = jdeDecryptCreds({ ...sRow.data, tenant_id: tid });
        if (!jdeIsConfigured(settings)) continue;
        out.push({
          tenant_id: tid,
          ...await drainRetryQueue(svc, "jde", {
            tenantId: tid,
            opts: {},
    ...
    if (!jdeIsConfigured(settings)) {
      return json(res, 409, { error: { code: "JDE_NOT_CONFIGURED", message: "JDE not configured" } });
    }
    const r = await drainRetryQueue(svc, "jde", {
