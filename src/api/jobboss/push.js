// POST /api/jobboss/push  Body: { orderId, dry_run? }
// Phase 5.4b cluster B.
...
import { jobbossDecryptCreds, jobbossPushSalesOrder, jobbossIsConfigured } from "../_lib/jobboss-client.js";
...
const enqueueRetry = async (svc, tenantId, orderId, payload, status, err) => {
  await svc.from("jobboss_retry_queue").insert({
    tenant_id: tenantId,
    order_id: orderId,
    payload,
    attempt_count: 1,
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!jobbossIsConfigured(settings)) {
      return json(res, 409, { error: { code: "JOBBOSS_NOT_CONFIGURED", message: "JobBoss not configured" } });
    }
    ...
    const idem = await checkExportIdempotency(svc, {
      tenantId: ctx.tenantId, orderId: orderQ.data.id, connector: "jobboss",
      payloadHash, allowReexport: body.reexport === true,
    });
    if (idem.idempotent) return json(res, 200, { ok: true, idempotent: true, external_id: idem.external_id, jobboss_id: idem.external_id });
    if (idem.blocked) return json(res, idem.status, idem.body);

    let resp = null;
    try {
      resp = await jobbossPushSalesOrder(settings, orderForPush, settings.jobboss_field_map || {});
    } catch (err) {
    ...
    await recordAudit(ctx, {
      action: ok ? "jobboss_push" : "jobboss_push_failed",
      objectType: "order",
      objectId: orderQ.data.id,
      detail: ok ? ("jobboss_id=" + externalId) : ("status=" + resp.status),
    });
    ...
    await recordExport(svc, { tenantId: ctx.tenantId, orderId: orderQ.data.id, connector: "jobboss", payloadHash, externalId });
    return json(res, 200, { ok: true, jobboss_id: externalId, status: resp.status });
  } catch (err) {
