// /api/jobboss/retry
// Drain the JobBoss retry queue with exponential backoff. Phase 5.4b cluster B.

import { applyCors, handlePreflight, json, readBody, sendError } from "../_lib/cors.js";
import { resolveContext, requirePermission } from "../_lib/auth.js";
import { serviceClient } from "../_lib/supabase.js";
import { recordAudit } from "../_lib/audit.js";
import { jobbossDecryptCreds, jobbossIsConfigured, jobbossPushSalesOrder } from "../_lib/jobboss-client.js";
...
const replayFor = (svc, settings, ctx) => async (row) => {
  ...
  let resp;
  try {
    resp = await jobbossPushSalesOrder(settings, { ...orderQ.data, customer }, settings.jobboss_field_map || {});
  } catch (err) {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (isCron) {
      const tenants = await svc.from("jobboss_retry_queue").select("tenant_id")
        .eq("status", "pending")
      ...
      for (const tid of uniq) {
        ...
        const settings = jobbossDecryptCreds({ ...sRow.data, tenant_id: tid });
        if (!jobbossIsConfigured(settings)) continue;
        out.push({
          tenant_id: tid,
          ...await drainRetryQueue(svc, "jobboss", {
            tenantId: tid,
            opts: {},
    ...
    const r = await drainRetryQueue(svc, "jobboss", {
