// /api/oracle_ebs/retry — Phase 5.4b cluster C.

import { applyCors, handlePreflight, json, readBody, sendError } from "../_lib/cors.js";
import { resolveContext, requirePermission } from "../_lib/auth.js";
import { serviceClient } from "../_lib/supabase.js";
import { recordAudit } from "../_lib/audit.js";
import { oracleEbsDecryptCreds, oracleEbsIsConfigured, oracleEbsPushSalesOrder } from "../_lib/oracle-ebs-client.js";
...
const replayFor = (svc, settings, ctx) => async (row) => {
  ...
  let resp;
  try {
    resp = await oracleEbsPushSalesOrder(settings, { ...orderQ.data, customer }, settings.oracle_ebs_field_map || {});
  } catch (err) {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (isCron) {
      const tenants = await svc.from("oracle_ebs_retry_queue").select("tenant_id")
        .eq("status", "pending")
      ...
      for (const tid of uniq) {
        ...
        const settings = oracleEbsDecryptCreds({ ...sRow.data, tenant_id: tid });
        if (!oracleEbsIsConfigured(settings)) continue;
        out.push({
          tenant_id: tid,
          ...await drainRetryQueue(svc, "oracle_ebs", {
            tenantId: tid,
            opts: {},
    ...
    const r = await drainRetryQueue(svc, "oracle_ebs", {
