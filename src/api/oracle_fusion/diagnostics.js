// GET /api/oracle_fusion/diagnostics
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { probes, summary } = await runConnectorDiagnostics(oracleFusionFetch, settings, PROBES);
    return json(res, 200, { configured: true, base_url: settings.oracle_fusion_base_url, probes, summary, ran_at: new Date().toISOString() });
