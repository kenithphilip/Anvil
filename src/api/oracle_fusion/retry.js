// /api/oracle_fusion/retry
// Drain the Oracle Fusion retry queue with exponential backoff.

import { applyCors, handlePreflight, json, readBody, sendError } from "../_lib/cors.js";
import { resolveContext, requirePermission } from "../_lib/auth.js";
import { serviceClient } from "../_lib/supabase.js";
import { recordAudit } from "../_lib/audit.js";
import { oracleFusionDecryptCreds, oracleFusionIsConfigured, oracleFusionPushSalesOrder } from "../_lib/oracle-fusion-client.js";
...
const replayFor = (svc, settings, ctx) => async (row) => {
  ...
  let resp;
  try {
    resp = await oracleFusionPushSalesOrder(settings, { ...orderQ.data, customer }, settings.oracle_fusion_field_map || {});
  } catch (err) {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (isCron) {
      const tenants = await svc.from("oracle_fusion_retry_queue").select("tenant_id")
        .eq("status", "pending")
      ...
      for (const tid of uniq) {
        ...
        const settings = oracleFusionDecryptCreds({ ...sRow.data, tenant_id: tid });
        if (!oracleFusionIsConfigured(settings)) continue;
        out.push({
          tenant_id: tid,
          ...await drainRetryQueue(svc, "oracle_fusion", {
            tenantId: tid,
            opts: {},
    ...
    const r = await drainRetryQueue(svc, "oracle_fusion", {
