// POST /api/plex/push  Body: { orderId, dry_run? }
...
import { plexDecryptCreds, plexPushSalesOrder, plexIsConfigured } from "../_lib/plex-client.js";
...
const enqueueRetry = async (svc, tenantId, orderId, payload, status, err) => {
  await svc.from("plex_retry_queue").insert({
    tenant_id: tenantId,
    order_id: orderId,
    payload,
    attempt_count: 1,
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!plexIsConfigured(settings)) {
      return json(res, 409, { error: { code: "PLEX_NOT_CONFIGURED", message: "Plex not configured" } });
    }
    ...
    const idem = await checkExportIdempotency(svc, {
      tenantId: ctx.tenantId, orderId: orderQ.data.id, connector: "plex",
      payloadHash, allowReexport: body.reexport === true,
    });
    if (idem.idempotent) return json(res, 200, { ok: true, idempotent: true, external_id: idem.external_id, plex_id: idem.external_id });
    if (idem.blocked) return json(res, idem.status, idem.body);

    let resp = null;
    try {
      resp = await plexPushSalesOrder(settings, orderForPush, settings.plex_field_map || {});
    } catch (err) {
    ...
    await recordAudit(ctx, {
      action: ok ? "plex_push" : "plex_push_failed",
      objectType: "order",
      objectId: orderQ.data.id,
      detail: ok ? ("plex_id=" + externalId) : ("status=" + resp.status),
    });
    ...
    await recordExport(svc, { tenantId: ctx.tenantId, orderId: orderQ.data.id, connector: "plex", payloadHash, externalId });
    return json(res, 200, { ok: true, plex_id: externalId, status: resp.status });
  } catch (err) {
