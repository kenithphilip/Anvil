// /api/plex/retry
// Drain the Plex retry queue with exponential backoff. Phase 5.4b cluster B.

import { applyCors, handlePreflight, json, readBody, sendError } from "../_lib/cors.js";
import { resolveContext, requirePermission } from "../_lib/auth.js";
import { serviceClient } from "../_lib/supabase.js";
import { recordAudit } from "../_lib/audit.js";
import { plexDecryptCreds, plexIsConfigured, plexPushSalesOrder } from "../_lib/plex-client.js";
...
const replayFor = (svc, settings, ctx) => async (row) => {
  ...
  let resp;
  try {
    resp = await plexPushSalesOrder(settings, { ...orderQ.data, customer }, settings.plex_field_map || {});
  } catch (err) {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (isCron) {
      const tenants = await svc.from("plex_retry_queue").select("tenant_id")
        .eq("status", "pending")
      ...
      for (const tid of uniq) {
        ...
        const settings = plexDecryptCreds({ ...sRow.data, tenant_id: tid });
        if (!plexIsConfigured(settings)) continue;
        out.push({
          tenant_id: tid,
          ...await drainRetryQueue(svc, "plex", {
            tenantId: tid,
            opts: {},
