// POST /api/proalpha/push  Body: { orderId, dry_run? }
...
import { proalphaDecryptCreds, proalphaPushSalesOrder, proalphaIsConfigured } from "../_lib/proalpha-client.js";
...
const enqueueRetry = async (svc, tenantId, orderId, payload, status, err) => {
  await svc.from("proalpha_retry_queue").insert({
    tenant_id: tenantId,
    order_id: orderId,
    payload,
    attempt_count: 1,
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!proalphaIsConfigured(settings)) {
      return json(res, 409, { error: { code: "PROALPHA_NOT_CONFIGURED", message: "proALPHA not configured" } });
    }
    ...
    if (body.dry_run) {
      const preview = {
        ...
        currency: orderQ.data.result?.salesOrder?.currency || "EUR",
      };
    ...
    const idem = await checkExportIdempotency(svc, {
      tenantId: ctx.tenantId, orderId: orderQ.data.id, connector: "proalpha",
      payloadHash, allowReexport: body.reexport === true,
    });
    if (idem.idempotent) return json(res, 200, { ok: true, idempotent: true, external_id: idem.external_id, proalpha_id: idem.external_id });
    if (idem.blocked) return json(res, idem.status, idem.body);

    let resp = null;
    try {
      resp = await proalphaPushSalesOrder(settings, orderForPush, settings.proalpha_field_map || {});
    } catch (err) {
    ...
    await recordAudit(ctx, {
      action: ok ? "proalpha_push" : "proalpha_push_failed",
      objectType: "order",
      objectId: orderQ.data.id,
      detail: ok ? ("proalpha_id=" + externalId) : ("status=" + resp.status),
    });
    ...
    await recordExport(svc, { tenantId: ctx.tenantId, orderId: orderQ.data.id, connector: "proalpha", payloadHash, externalId });
    return json(res, 200, { ok: true, proalpha_id: externalId, status: resp.status });
  } catch (err) {
