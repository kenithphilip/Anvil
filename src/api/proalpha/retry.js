// /api/proalpha/retry — Phase 5.4b cluster C.

import { applyCors, handlePreflight, json, readBody, sendError } from "../_lib/cors.js";
import { resolveContext, requirePermission } from "../_lib/auth.js";
import { serviceClient } from "../_lib/supabase.js";
import { recordAudit } from "../_lib/audit.js";
import { proalphaDecryptCreds, proalphaIsConfigured, proalphaPushSalesOrder } from "../_lib/proalpha-client.js";
...
const replayFor = (svc, settings, ctx) => async (row) => {
  ...
  let resp;
  try {
    resp = await proalphaPushSalesOrder(settings, { ...orderQ.data, customer }, settings.proalpha_field_map || {});
  } catch (err) {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (isCron) {
      const tenants = await svc.from("proalpha_retry_queue").select("tenant_id")
        .eq("status", "pending")
      ...
      for (const tid of uniq) {
        ...
        const settings = proalphaDecryptCreds({ ...sRow.data, tenant_id: tid });
        if (!proalphaIsConfigured(settings)) continue;
        out.push({
          tenant_id: tid,
          ...await drainRetryQueue(svc, "proalpha", {
            tenantId: tid,
            opts: {},
    ...
    const r = await drainRetryQueue(svc, "proalpha", {
