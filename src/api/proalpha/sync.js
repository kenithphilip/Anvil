// /api/proalpha/sync — Phase 5.4b cluster C.
...
const ENTITY = {
  ...
  item: {
    ...
    upsert: async (svc, tid, items) => {
      let updated = 0; let hw = null;
      for (const r of items) {
        await svc.from("proalpha_items").upsert({
          tenant_id: tid,
          external_id: String(r.articleNumber || r.id),
  ...
  sales_order: {
    ...
    upsert: async (svc, tid, items) => {
      let updated = 0; let hw = null;
      for (const r of items) {
        await svc.from("proalpha_sales_orders").upsert({
          tenant_id: tid,
          external_id: String(r.orderNumber || r.id),
          customer_external_id: r.customer ? String(r.customer) : null,
...
const reverseSyncSalesOrders = async (svc, tenantId, settings) => {
  const orders = await svc.from("orders").select("id, result")
    ...
    .not("result->external_systems->proalpha", "is", null)
    .limit(500);
  if (orders.error) return { entity: "sales_order_status", error: orders.error.message };
  const linked = (orders.data || []).filter((o) => o.result?.external_systems?.proalpha?.external_id);
  if (!linked.length) return { entity: "sales_order_status", pulled: 0 };
  let updated = 0;
  for (const o of linked) {
    const id = encodeURIComponent(o.result.external_systems.proalpha.external_id);
    const r = await proalphaFetch(settings, {
      method: "GET",
      path: `salesOrder/${id}`,
    }).catch(() => null);
    if (!r?.ok) continue;
    const newResult = {
      ...
      external_systems: {
        ...
        proalpha: {
          ...o.result.external_systems.proalpha,
...
const runForTenant = async (svc, tenantId, settingsRow, opts) => {
  const settings = proalphaDecryptCreds({ ...settingsRow, tenant_id: tenantId });
  ...
  for (const entity of which) {
    ...
    out.push(await runSyncEntity(svc, PREFIX, {
      ...
      runner: async (since) => {
        ...
        const items = await proalphaList(settings, def.path, { filter, top: 200, maxRows: 5000 });
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (isCron) {
      const tenants = await svc.from("tenant_settings").select("*").not("proalpha_base_url", "is", null);
