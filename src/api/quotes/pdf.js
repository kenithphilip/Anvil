...
// Audit fix May 2026: the client helper at
// src/v3-app/lib/amount-words.ts accepts { style: "intl" | "indian" }
// so Indian-distributor tenants can render "Two Lakh Thirty Thousand"
// instead of "Two Hundred Thirty Thousand". The server helper here
// only supported intl, so the SO/quote PDF diverged from the
// recon-table preview the operator saw. Mirror the styles.
const amountInWords = (raw, currencyOrOpts = "INR") => {
  ...
  if (opts.style === "indian") {
    // Indian grouping: 2-digit groups after the first 3 digits.
    // Order: (Crore) (Lakh) (Thousand) (last 3).
    let rem = rupees;
    const last3 = rem % 1000;
    rem = Math.floor(rem / 1000);
    const thousand = rem % 100;
    ...
    const units = ["", "Thousand", "Million", "Billion"]; let rem = rupees; let i = 0;
