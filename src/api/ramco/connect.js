// POST /api/ramco/connect
//
// Stores Ramco ERP credentials on tenant_settings (encrypted when
// secrets are configured) and runs a probe call to validate them.
// Phase 5.4b cluster A.
...
import { ramcoEncryptCreds, ramcoDecryptCreds, ramcoProbe } from "../_lib/ramco-client.js";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const enc = ramcoEncryptCreds({ client_id: body.client_id, client_secret: body.client_secret });
    const updated = await updateTenantSettings(svc, ctx.tenantId, {
      ramco_base_url: String(body.base_url).replace(/\/+$/, ""),
      ramco_token_url: body.token_url,
      ramco_scope: body.scope || "api",
      ramco_org_unit: body.org_unit || null,
      ramco_company: body.company || null,
      ...enc,
    });
    const decrypted = ramcoDecryptCreds({ ...updated, tenant_id: ctx.tenantId });

    let probe = null;
    let probeErr = null;
    try {
      probe = await ramcoProbe(decrypted);
    } catch (err) {
      probeErr = err.message;
      probe = { ok: false, status: 0, body: { error: err.message } };
    }
    if (probe.ok) {
      await updateTenantSettings(svc, ctx.tenantId, { ramco_connected_at: new Date().toISOString() });
    }
    await recordAudit(ctx, {
      action: "ramco_connect",
