// /api/ramco/sync
//
// Pulls customers, items, sales orders from Ramco ERP into the
// local mirror tables. Cron-driven for every configured tenant;
// admin-callable to trigger an immediate refresh.

import { applyCors, handlePreflight, json, readBody, sendError } from "../_lib/cors.js";
import { resolveContext, requirePermission } from "../_lib/auth.js";
import { serviceClient } from "../_lib/supabase.js";
import { ramcoDecryptCreds, ramcoList, ramcoIsConfigured, ramcoFetch } from "../_lib/ramco-client.js";
...
const ENTITY = {
  customer: {
    resource: "Common/Customer", cursorCol: "lastModifiedDate",
    upsert: async (svc, tid, items) => {
      let updated = 0; let hw = null;
      for (const r of items) {
        const externalId = String(r.CustomerCode || r.customerId || r.id);
        const name = r.CustomerName || r.name || null;
        if (name) {
          // Audit P8.2: promote to canonical customers table.
          await canonicaliseCustomer(svc, tid, {
            vendor: "ramco",
            vendorIdField: "ramco_id",
            externalId,
            name,
            email: r.EmailAddress || r.email || null,
            currency: r.Currency || null,
            ref: { status: r.Status, modified: r.lastModifiedDate || r.LastModifiedDate },
          });
        }
        await svc.from("ramco_customers").upsert({
          tenant_id: tid,
        ...
        updated += 1;
        const t = r.lastModifiedDate || r.LastModifiedDate;
  ...
  item: {
    resource: "Inventory/Part", cursorCol: "lastModifiedDate",
    upsert: async (svc, tid, items) => {
      let updated = 0; let hw = null;
      for (const r of items) {
        await svc.from("ramco_items").upsert({
          tenant_id: tid,
          external_id: String(r.PartNumber || r.partId || r.id),
  ...
  sales_order: {
    resource: "Sales/SalesOrder", cursorCol: "lastModifiedDate",
    upsert: async (svc, tid, items) => {
      let updated = 0; let hw = null;
      for (const r of items) {
        await svc.from("ramco_sales_orders").upsert({
          tenant_id: tid,
          external_id: String(r.SalesOrderNumber || r.id),
          ...
          currency: r.Currency || null,
          total: r.TotalValue != null ? Number(r.TotalValue) : null,
          raw: r,
          synced_at: new Date().toISOString(),
        }, { onConflict: "tenant_id,external_id" });
        updated += 1;
...
const reverseSyncSalesOrders = async (svc, tenantId, settings) => {
  const orders = await svc.from("orders").select("id, result")
    ...
    .not("result->external_systems->ramco", "is", null)
    .limit(500);
  if (orders.error) return { entity: "sales_order_status", error: orders.error.message };
  const linked = (orders.data || []).filter((o) => o.result?.external_systems?.ramco?.external_id);
  if (!linked.length) return { entity: "sales_order_status", pulled: 0 };
  let updated = 0;
  for (const o of linked) {
    const id = encodeURIComponent(o.result.external_systems.ramco.external_id);
    const r = await ramcoFetch(settings, {
      method: "GET",
      resource: `Sales/SalesOrder/${id}`,
    }).catch(() => null);
    if (!r?.ok) continue;
    const newResult = {
      ...o.result,
      external_systems: {
        ...
        ramco: {
          ...o.result.external_systems.ramco,
...
const runForTenant = async (svc, tenantId, settingsRow, opts) => {
  const settings = ramcoDecryptCreds({ ...settingsRow, tenant_id: tenantId });
  ...
  for (const entity of which) {
    ...
    out.push(await runSyncEntity(svc, PREFIX, {
      ...
      runner: async (since) => {
        ...
        const items = await ramcoList(settings, def.resource, { filter, top: 200, maxRows: 5000 });
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (isCron) {
      const tenants = await svc.from("tenant_settings").select("*").not("ramco_base_url", "is", null);
