...
const PROBES = [
  { entity: "customer", path: "/M3/m3api-rest/v2/customer" },
  { entity: "item", path: "/M3/m3api-rest/v2/item" },
  { entity: "sales_order", path: "/M3/m3api-rest/v2/customer-order" },
  { entity: "purchase_order", path: "/M3/m3api-rest/v2/purchase-order" },
  { entity: "warehouse", path: "/M3/m3api-rest/v2/warehouse" },
  { entity: "inventory", path: "/M3/m3api-rest/v2/inventory-balance" },
];
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!sxeIsConfigured(settings)) {
      return json(res, 200, { configured: false, probes: [], notes: ["SX.e not configured"] });
    }
    const out = [];
    for (const p of PROBES) {
      const t0 = Date.now();
      try {
        const r = await sxeFetch(settings, { method: "GET", path: p.path, query: { $top: "1" } });
        out.push({
    ...
    return json(res, 200, {
      configured: true,
      base_url: settings.sxe_base_url,
      probes: out,
