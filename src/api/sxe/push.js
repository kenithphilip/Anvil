// POST /api/sxe/push  Body: { orderId, dry_run? }
...
const buildPayload = (order, customer, settings) => {
  ...
  const map = settings?.sxe_field_map || {};
  const lines = (so.lineItems || []).map((li, idx) => ({
    lineNumber: idx + 1,
    itemNumber: li.partNumber || li.itemName || "",
    quantity: Number(li.quantity || li.qty || 1),
    unitPrice: Number(li.rate || li.unitPrice || 0),
  }));
  const payload = {
    customerNumber: customer?.external_ref?.sxe_id || customer?.customer_key || "",
    warehouseId: settings?.sxe_default_warehouse || null,
    customerOrderNumber: order.po_number || null,
...
const enqueueRetry = async (svc, tenantId, orderId, payload, status, err) => {
  await svc.from("sxe_retry_queue").insert({
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!sxeIsConfigured(settings)) {
      return json(res, 409, { error: { code: "SXE_NOT_CONFIGURED", message: "Infor SX.e not configured" } });
    }
    ...
    const idem = await checkExportIdempotency(svc, {
      tenantId: ctx.tenantId, orderId: orderQ.data.id, connector: "sxe",
      payloadHash, allowReexport: body.reexport === true,
    });
    if (idem.idempotent) return json(res, 200, { ok: true, idempotent: true, external_id: idem.external_id, sxe_id: idem.external_id });
    if (idem.blocked) return json(res, idem.status, idem.body);
    let resp = null;
    try {
      resp = await sxeFetch(settings, { method: "POST", path: "/M3/m3api-rest/v2/customer-order", body: payload });
    } catch (err) {
    ...
    await recordAudit(ctx, {
      action: ok ? "sxe_push" : "sxe_push_failed",
      objectType: "order", objectId: orderQ.data.id,
      detail: ok ? ("sxe_id=" + externalId) : ("status=" + resp.status),
    });
    ...
    await recordExport(svc, { tenantId: ctx.tenantId, orderId: orderQ.data.id, connector: "sxe", payloadHash, externalId });
    return json(res, 200, { ok: true, sxe_id: externalId, status: resp.status });
