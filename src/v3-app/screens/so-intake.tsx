...
const WiredSOIntake = () => {
  ...
  // Phases A-C cover six adapters (claude / reducto / azure_di /
  // unstructured / docling / marker); plus the legacy "anthropic"
  // alias kept for back-compat. Per-tenant settings can flip an
  // adapter on without an env var, so this is a "platform has
  // *some* adapter" check, not "this tenant is fully wired."
  const docaiConfigured = !!health?.integrations?.some?.((i: any) =>
    ["claude", "anthropic", "reducto", "azure_di", "unstructured", "docling", "marker", "docai"].includes(i.id) && i.configured
  );
