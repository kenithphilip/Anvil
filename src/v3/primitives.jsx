// ============================================================
// ANVIL v3 — Primitives
// Shared building blocks for the operator console.
// ============================================================

const { useState, useEffect, useRef, useMemo } = React;

// ---------- Inline icons (Lucide-ish, hand-tuned) ------------
const I = ({ d, size = 14, sw = 1.5, fill = "none", style }) => (
...
// ---------- Buttons / chips ---------------------------------
const Btn = ({ children, kind, sm, lg, icon, full, onClick, disabled, type = "button", className = "" }) => {
...
const Chip = ({ children, k, lg }) => <span className={["chip", k, lg && "lg"].filter(Boolean).join(" ")}>{children}</span>;
const Dot = ({ k }) => <span className={["dot", k].filter(Boolean).join(" ")} />;
const Sev = ({ k = "low" }) => <span className={`sev ${k}`} />;
const Prov = ({ children }) => <span className="prov">{children}</span>;

// ---------- Workspace shell pieces --------------------------
const WSTitle = ({ eyebrow, title, meta, right }) => (
...
const WSTabs = ({ tabs, active, onChange }) => (
  <div className="ws-tabs">
    {tabs.map(t => (
      <div key={t.id} className={`ws-tab ${active === t.id ? "active" : ""}`} onClick={() => onChange?.(t.id)}>
...
const Card = ({ title, eyebrow, right, children, flush, className = "", style }) => (
...
const RailPanel = ({ title, count, children, action }) => (
  ...
  <div className="stream">
    {rows.map((r, i) => (
      <div key={i} className="stream-row">
        ...
        <span className="m" dangerouslySetInnerHTML={{ __html: r.m }} />
      </div>
    ))}
  </div>
);
