...
const WiredAdminCRUD = () => {
  ...
  const holidays = useFetch(
    () => window.ObaraBackend?.admin?.listHolidays?.()
          || fetch("/api/admin/holidays").then((r) => r.ok ? r.json() : { holidays: [] }).catch(() => ({ holidays: [] })),
    []
  );
  const leadTimes = useFetch(
    () => window.ObaraBackend?.admin?.listLeadTimes?.(leadTimeForm.type)
  ...
  const customers = useFetch(
    () => window.ObaraBackend?.customers?.list?.() || adminCrudFetch("/api/customers"),
    []
  );
  const locations = useFetch(
    () => window.ObaraBackend?.admin?.listCustomerLocations?.() || adminCrudFetch("/api/admin/customer_locations"),
    []
  );
  const contracts = useFetch(
    () => window.ObaraBackend?.admin?.listContracts?.() || adminCrudFetch("/api/admin/contracts"),
    []
  );
  const itemMaster = useFetch(
    () => window.ObaraBackend?.admin?.listItemMaster?.({ limit: 500 })
          || adminCrudFetch("/api/admin/item_master?limit=500"),
    []
  );
  const diagnostics = useFetch(
    () => window.ObaraBackend?.admin?.diagnostics?.()
  ...
  const onAddHoliday = async (ev) => {
    ...
    setBusy(true); setFlash(null);
    try {
      await (window.ObaraBackend?.admin?.upsertHoliday?.(holidayForm)
  ...
  const onDeleteHoliday = async (id) => {
    ...
    setBusy(true); setFlash(null);
    try {
      await (window.ObaraBackend?.admin?.deleteHoliday?.(id)
  ...
  const onAddLeadTime = async (ev) => {
    ...
    try {
      ...
      await (window.ObaraBackend?.admin?.upsertLeadTime?.(leadTimeForm.type, payload)
  ...
  const onDeleteLeadTime = async (id) => {
    ...
    setBusy(true); setFlash(null);
    try {
      await (window.ObaraBackend?.admin?.deleteLeadTime?.(leadTimeForm.type, id)
  ...
  const submitThreshold = async () => {
    ...
    setBusy(true); setFlash(null);
    try {
      await (window.ObaraBackend?.admin?.upsertApprovalThreshold?.(threshForm)
  ...
  const deleteThreshold = async (id) => {
    ...
    setBusy(true); setFlash(null);
    try {
      await (window.ObaraBackend?.admin?.deleteApprovalThreshold?.(id)
  ...
  const submitLocation = async () => {
    ...
    setBusy(true); setFlash(null);
    try {
      await (window.ObaraBackend?.admin?.upsertCustomerLocation?.(locForm)
  ...
  const deleteLocation = async (id) => {
    ...
    setBusy(true); setFlash(null);
    try {
      await (window.ObaraBackend?.admin?.deleteCustomerLocation?.(id)
  ...
  const submitContract = async () => {
    ...
    setBusy(true); setFlash(null);
    try {
      await (window.ObaraBackend?.admin?.upsertContract?.(contractForm)
  ...
  const deleteContract = async (id) => {
    ...
    setBusy(true); setFlash(null);
    try {
      await (window.ObaraBackend?.admin?.deleteContract?.(id)
  ...
  const submitItem = async () => {
    ...
    setBusy(true); setFlash(null);
    try {
      await (window.ObaraBackend?.admin?.upsertItemMaster?.(itemForm)
  ...
  const deleteItem = async (id) => {
    ...
    setBusy(true); setFlash(null);
    try {
      await (window.ObaraBackend?.admin?.deleteItemMaster?.(id)
  ...
  const onCsvImport = async (file) => {
    ...
    try {
      ...
      await (window.ObaraBackend?.admin?.bulkItemMaster?.(items)
          ...
          <>
            ...
            <Card title="Tenant" eyebrow="local browser snapshot">
              <KV rows={[
                ...
                ["Backend ready", String(!!window.ObaraBackend?.isReady?.())],
...
window.AdminCenter = WiredAdminCRUD;
