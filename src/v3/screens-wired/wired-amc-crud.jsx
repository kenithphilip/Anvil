...
const WiredAmcCRUD = () => {
  ...
  const canWrite = window.RBAC?.canDo?.("service.write") ?? true;
  const canAdmin = window.RBAC?.canDo?.("service.admin") ?? canWrite;

  const reload = () => {
    setList((s) => ({ ...s, loading: true }));
    Promise.resolve(
      window.ObaraBackend?.service?.listAmcSchedules?.()
  ...
  e(() => {
    Promise.resolve(
      window.ObaraBackend?.admin?.listContracts?.()
  ...
  e(() => {
    Promise.resolve(
      window.ObaraBackend?.customers?.list?.()
  ...
  e(() => {
    ...
    Promise.resolve(
      window.ObaraBackend?.admin?.listCustomerLocations?.(form.customer_id)
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (editing && editing !== "__new__") {
        ...
        await (window.ObaraBackend?.service?.updateAmcSchedule?.(body)
               || amcCrudFetch("/api/service/amc", { method: "PATCH", body }));
        setOkMsg("Schedule updated");
      } else {
        await (window.ObaraBackend?.service?.createAmcSchedule?.(payload)
  ...
  const submitSeed = async () => {
    ...
    try {
      ...
      await (window.ObaraBackend?.service?.bulkSeedAmcSchedule?.(payload)
  ...
  const generateVisit = async (id) => {
    ...
    setErr(null);
    try {
      await (window.ObaraBackend?.service?.generateAmcVisit?.(id)
  ...
  const removeRow = async (id) => {
    ...
    setErr(null);
    try {
      await (window.ObaraBackend?.service?.deleteAmcSchedule?.(id)
  ...
  const cancelSchedule = async (id) => {
    ...
    setErr(null);
    try {
      await (window.ObaraBackend?.service?.updateAmcSchedule?.({ id, status: "CANCELLED" })
...
window.AMCSchedule = WiredAmcCRUD;
