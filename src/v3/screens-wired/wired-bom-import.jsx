...
const fetchExistingBomChildren = async (gunNo) => {
  try {
    ...
    if (!window.ObaraBackend?.bom?.list) return null;
    const resp = await window.ObaraBackend.bom.list({ parent: gunNo });
...
const WiredBomImport = () => {
  const [files, setFiles] = useStateW([]);          // [{id, name, origin, gunNo, items, status, error, expanded, diff}]
  const [importing, setImporting] = useStateW(false);
  const [progress, setProgress] = useStateW({ done: 0, total: 0, current: "" });
  const [logLines, setLogLines] = useStateW([]);
  const [parserLoading, setParserLoading] = useStateW(false);
  const [dragActive, setDragActive] = useStateW(false);
  ...
  const doImportAll = async () => {
    ...
    for (let i = 0; i < importable.length; i++) {
      ...
      try {
        const resp = await window.ObaraBackend?.bom?.upsert?.({ rows });
...
window.BomImport = WiredBomImport;
