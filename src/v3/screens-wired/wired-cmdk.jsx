...
const WiredCmdK = ({ open, onClose, onJump }) => {
  ...
  uE(() => {
    ...
    const t = setTimeout(async () => {
      try {
        // Item search uses a direct fetch since admin.listItemMaster
        // isn't exposed on the client wrapper in all builds. Pull a
        // small page and filter client-side.
        const itemFetch = async () => {
          const cfg = (() => { try { return JSON.parse(localStorage.getItem("obara:backend_config") || "{}"); } catch { return {}; } })();
          const session = (() => { try { return JSON.parse(localStorage.getItem("obara:backend_session") || "null"); } catch { return null; } })();
          if (!cfg.url) return [];
          const headers = { "Content-Type": "application/json" };
          if (session?.access_token) headers["Authorization"] = "Bearer " + session.access_token;
          if (cfg.tenantId) headers["x-obara-tenant"] = cfg.tenantId;
          const u = cfg.url.replace(/\/+$/, "") + "/api/admin/item_master?limit=200";
          const r = await fetch(u, { headers });
          if (!r.ok) return [];
          const j = await r.json();
          return Array.isArray(j) ? j : (j?.rows || j?.items || []);
        };
        ...
        const iList = Array.isArray(items) ? items : (items?.rows || items?.items || []);
        ...
        setResults({
          ...
          items: iList.filter((it) =>
            (it.part_no || "").toLowerCase().includes(ql) ||
            (it.description || "").toLowerCase().includes(ql) ||
            (it.drawing_no || "").toLowerCase().includes(ql)
          ).slice(0, 6),
          loading: false,
        });
      } catch (err) {
      ...
      if (results.items.length) {
        out.push({
          ...
          items: results.items.map((it) => ({
            ic: window.Icon?.tag,
            t: `${it.part_no} · ${it.description || "—"}`,
            m: it.source_country || it.hsn_sac || "",
            onPick: () => { window.location.hash = `#/items?part=${encodeURIComponent(it.part_no)}`; onJump?.("items"); onClose?.(); },
          })),
        });
      }
