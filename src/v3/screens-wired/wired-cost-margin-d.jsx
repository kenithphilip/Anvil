...
// ---------- Breakdown tab --------------------------------
const CostBreakdown = () => {
  const breakdown = useFetch(() => window.ObaraBackend?.cost?.breakdown?.() || Promise.resolve({}), []);
...
// ---------- Simulator tab --------------------------------
const CostSimulator = () => {
  const [scenarioId, setScenarioId] = useStateW(COST_SCENARIOS[0].id);
  const [busy, setBusy] = useStateW(false);
  const [resp, setResp] = useStateW(null);
  ...
  useEffectW(() => {
    ...
    setBusy(true);
    setErr(null);
    Promise.resolve(window.ObaraBackend?.cost?.simulator?.({ tokenEstimate: SIMULATOR_TOKEN_ESTIMATE }))
...
const CostMarginHistory = () => {
  ...
  const [history, setHistory] = useStateW({ data: null, error: null, loading: false });
  ...
  useEffectW(() => {
    ...
    Promise.resolve(window.ObaraBackend?.cost?.marginHistory?.(customerId))
...
const WiredCostMargin = () => {
  const [active, setActive] = useStateW("breakdown");
  return (
    <>
      <WSTitle
...
window.CostMargin = WiredCostMargin;
