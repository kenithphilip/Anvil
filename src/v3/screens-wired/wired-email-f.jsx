...
const WiredEmailTriage = () => {
  ...
  const orders = useFetch(
    () => window.ObaraBackend?.orders?.list?.({ limit: 100 }) || Promise.resolve([]),
    []
  );
  ...
  const [selectedId, setSelectedId] = useStateW(rows[0]?.id || null);
  const [busy, setBusy] = useStateW(false);
  const [flash, setFlash] = useStateW(null);
  const [attachQuery, setAttachQuery] = useStateW("");
  ...
  const promote = async () => {
    ...
    try {
      await window.ObaraBackend?.orders?.create?.({ from_email_id: selected.id });
  ...
  const attachToOrder = async (orderId) => {
    ...
    setBusy(true); setFlash(null);
    try {
      await window.ObaraBackend?.orders?.update?.(orderId, { attached_email_id: selected.id });
  ...
  const requestMissing = async () => {
    ...
    try {
      ...
      await window.ObaraBackend?.communications?.missingDoc?.(orderId);
...
window.EmailTriage = WiredEmailTriage;
