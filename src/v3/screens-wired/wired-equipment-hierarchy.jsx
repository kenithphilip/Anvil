...
const EquipmentDetail = ({ node, customers, locations, onSave, onDelete, onCancel, busy }) => {
  ...
  const [parts, setParts] = useStateW(() => (seedRow?.installed_parts || []).map((p) => ({ ...p })));
  ...
  const validationError = useMemoW(() => {
  ...
  const filteredLocations = useMemoW(() => {
...
const WiredEquipmentHierarchy = () => {
  const equipment = useFetch(
    () => window.ObaraBackend?.admin?.listEquipment?.() || Promise.resolve({ equipment: [] }),
    []
  );
  ...
  const locationsList = useFetch(
    () => window.ObaraBackend?.admin?.listCustomerLocations?.() || Promise.resolve({ locations: [] }),
    []
  );

  const [filter, setFilter] = useStateW("");
  const [expanded, setExpanded] = useStateW({});
  const [selectedId, setSelectedId] = useStateW(null);
  ...
  const [selectedNode, setSelectedNode] = useStateW(null);
  ...
  const [optimistic, setOptimistic] = useStateW({});

  const customersById = useMemoW(() => {
    const m = new Map();
  ...
  const filteredEquipment = useMemoW(() => {
  ...
  const handleSave = async ({ equipment: row, installed_parts }) => {
    ...
    try {
      const resp = await window.ObaraBackend.admin.upsertEquipment({ ...row, installed_parts });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await window.ObaraBackend.admin.deleteEquipment(id);
...
window.EquipmentHierarchy = WiredEquipmentHierarchy;
