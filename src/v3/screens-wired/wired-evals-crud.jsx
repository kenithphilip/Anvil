...
const WiredEvalsCRUD = () => {
  ...
  const canWrite = window.RBAC?.canDo?.("evals.write") ?? true;
  const canAdmin = window.RBAC?.canDo?.("evals.admin") ?? canWrite;

  const reloadDash = () => {
    setDash((s) => ({ ...s, loading: true }));
    Promise.resolve(window.ObaraBackend?.eval?.dashboard?.(suiteFilter || undefined)
  ...
  const reloadCases = () => {
    ...
    Promise.resolve(window.ObaraBackend?.eval?.listCases?.(suiteFilter || undefined)
  ...
  const submit = async () => {
    ...
    try {
      ...
      await (window.ObaraBackend?.eval?.upsertCase?.(payload)
  ...
  const removeCase = async (id) => {
    ...
    setErr(null);
    try {
      await (window.ObaraBackend?.eval?.deleteCase?.(id)
  ...
  const submitRun = async () => {
    ...
    setRunResult(null);
    try {
      const result = await (window.ObaraBackend?.eval?.run?.(runForm.suite, runCases)
...
window.EvalSuites = WiredEvalsCRUD;
