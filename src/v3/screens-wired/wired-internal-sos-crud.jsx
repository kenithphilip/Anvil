...
const WiredInternalSosCRUD = () => {
  ...
  const reload = () => {
    ...
    Promise.resolve(window.ObaraBackend?.sales?.listInternalSos?.() || { rows: [] })
      .then((r) => {
  ...
  const submit = async () => {
    ...
    try {
      ...
      const fn = (editing && editing !== "__new__")
        ? window.ObaraBackend?.sales?.updateInternalSo
        : window.ObaraBackend?.sales?.createInternalSo;
      let result;
  ...
  const remove = async (id, ref) => {
    ...
    setBusy(true);
    try {
      const fn = window.ObaraBackend?.sales?.deleteInternalSo;
      if (typeof fn === "function") {
        await fn(id);
      } else {
        const cfg = JSON.parse(localStorage.getItem("obara:backend_config") || "{}");
...
window.InternalSOs = WiredInternalSosCRUD;
