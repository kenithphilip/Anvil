...
const WiredSOList = () => {
  ...
  e(() => {
    ...
    Promise.resolve(window.ObaraBackend?.orders?.list?.({ limit: 200 }) || [])
      .then((data) => {
        if (cancel) return;
    ...
    <>
      <WSTitle
        ...
        right={<>
          ...
          <Btn sm kind="ghost" onClick={() => setOrders((s) => ({ ...s, loading: true })) || window.ObaraBackend?.orders?.list?.({ limit: 200 }).then((d) => setOrders({ rows: Array.isArray(d) ? d : (d?.rows || []), loading: false, error: null }))}>{Icon.cycle} refresh</Btn>
