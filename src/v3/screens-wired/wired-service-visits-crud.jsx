...
const WiredServiceVisitsCRUD = () => {
  ...
  const reload = () => {
    ...
    Promise.resolve(window.ObaraBackend?.service?.listVisits?.() || svFetch("/api/service/visits"))
      .then((r) => {
  ...
  e(reload, []);
  e(() => {
    Promise.resolve(window.ObaraBackend?.customers?.list?.() || [])
      .then((r) => setCustomers(Array.isArray(r) ? r : (r?.rows || [])));
  }, []);
  e(() => {
    if (!form?.customer_id) { setLocations([]); return; }
    Promise.resolve(window.ObaraBackend?.admin?.listCustomerLocations?.(form.customer_id) || [])
      .then((r) => {
  ...
  const submit = async () => {
    ...
    try {
      const fn = (editing && editing !== "__new__")
        ? window.ObaraBackend?.service?.updateVisit
        : window.ObaraBackend?.service?.createVisit;
      const payload = { ...form };
  ...
  const checkIn = async (id) => {
    setBusy(true);
    try {
      const fn = window.ObaraBackend?.service?.updateVisit;
  ...
  const remove = async (id, label) => {
    ...
    setBusy(true);
    try {
      const fn = window.ObaraBackend?.service?.deleteVisit;
          ...
          {list.loading ? (
            ...
            <div className="body" style={{ padding: 22, textAlign: "center", color: "var(--ink-3)" }}>
              {list.rows.length === 0 ? "No visits planned yet. Use Plan visit." :
                <>No visits in this view. <a onClick={() => setActive("all")} style={{ color: "var(--ink)", cursor: "pointer", textDecoration: "underline" }}>show all</a></>}
            </div>
          ) : (
            <table className="tbl">
              <thead><tr>
...
window.ServiceVisits = WiredServiceVisitsCRUD;
