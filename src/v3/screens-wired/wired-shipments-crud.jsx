...
const WiredShipmentsCRUD = () => {
  ...
  const reload = () => {
    ...
    Promise.resolve(window.ObaraBackend?.sales?.listShipments?.() || { shipments: [] })
      .then((r) => {
  ...
  const submit = async () => {
    ...
    try {
      ...
      const fn = (editing && editing !== "__new__")
        ? (window.ObaraBackend?.sales?.updateShipment || window.ObaraBackend?.sales?.upsertShipment)
        : (window.ObaraBackend?.sales?.createShipment || window.ObaraBackend?.sales?.upsertShipment);
      let result;
  ...
  const remove = async (id, num) => {
    ...
    setBusy(true);
    try {
      const fn = window.ObaraBackend?.sales?.deleteShipment;
      if (typeof fn === "function") {
        await fn(id);
      } else {
        const cfg = JSON.parse(localStorage.getItem("obara:backend_config") || "{}");
  ...
  const setStatus = async (id, status) => {
    setBusy(true);
    try {
      const fn = window.ObaraBackend?.sales?.updateShipment || window.ObaraBackend?.sales?.upsertShipment;
      if (typeof fn === "function") {
        await fn({ id, status });
      } else {
        const cfg = JSON.parse(localStorage.getItem("obara:backend_config") || "{}");
...
window.Shipments = WiredShipmentsCRUD;
