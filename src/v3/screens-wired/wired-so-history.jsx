...
Object.assign(window, {
  parseSohFile, parseDelimited, parseXlsx, applySohImport, exportSoh,
  detectSoHistoryFormat, templateBlob, downloadTemplateBundle,
});
  ...
  const loadLive = uC(async () => {
    ...
    try {
      ...
      const results = await Promise.all(targets.map(async (t) => {
        try {
          const resp = await window.ObaraBackend?.salesHistory?.priceBand?.({ part_no: t.part_no });
          return { ...t, ...(resp || {}) };
        } catch (err) {
...
const ReverseSearchDrawer = ({ partNo, rows, onClose }) => {
  ...
  e(() => {
    ...
    Promise.resolve(window.ObaraBackend?.bom?.list?.({ part_no: partNo }) || [])
      .then((resp) => {
        if (cancel) return;
    ...
    Promise.resolve(window.ObaraBackend?.salesHistory?.priceBand?.({ part_no: partNo }) || null)
...
window.SOHistory = WiredSOHistory;
