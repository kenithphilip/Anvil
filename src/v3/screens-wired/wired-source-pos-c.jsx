...
const WiredSourcePOs = () => {
  ...
  const list = useFetch(() => window.ObaraBackend?.sourcePos?.list?.() || Promise.resolve([]), [bump]);
  const scorecard = useFetch(() => window.ObaraBackend?.sourcePos?.scorecard?.() || Promise.resolve([]), [bump]);
  ...
  const submitAck = async () => {
    ...
    try {
      ...
      await window.ObaraBackend?.sourcePos?.ack?.({ sourcePoId: ackPo.id, ack });
      closeAck();
      reload();
    } catch (err) {
      setSubmitErr(err);
...
window.SourcePOs = WiredSourcePOs;
window.SPOList = WiredSourcePOs;
