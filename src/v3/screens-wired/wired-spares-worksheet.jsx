...
const SMWorksheetPane = ({ matrix, onChange, onDelete, customers }) => {
  ...
  eM(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      onChange(next);
      try {
        if (next.customer_id && window.ObaraBackend?.spareMatrix?.recommend) {
          await window.ObaraBackend.spareMatrix.recommend({ customer_id: next.customer_id });
        }
        setSaveState("saved");
  ...
  const onAutoFill = async () => {
    setBusyAuto(true);
    try {
      const resp = await window.ObaraBackend?.bom?.list?.();
  ...
  const onSyncRecommended = async () => {
    ...
    try {
      ...
      onChange(next);
      setDraft(next);
      if (draft.customer_id && window.ObaraBackend?.spareMatrix?.recommend) {
        await window.ObaraBackend.spareMatrix.recommend({ customer_id: draft.customer_id });
      }
...
const SMWorksheetTab = () => {
  ...
  const onCreate = async ({ customer_id, project_name, name }) => {
    ...
    setShowNew(false);
    try {
      if (window.ObaraBackend?.spareMatrix?.recommend) {
        await window.ObaraBackend.spareMatrix.recommend({ customer_id });
      }
    } catch (err) {
...
const SMSubTab = ({ tab, customerId, customers, onCustomerChange }) => {
  ...
  const run = async () => {
    ...
    try {
      let data;
      if (tab === "recommend")     data = await window.ObaraBackend?.spareMatrix?.recommend?.({ customer_id: customerId });
      else if (tab === "kit")      data = await window.ObaraBackend?.spareMatrix?.kit?.({ customer_id: customerId, months: Number(months) || 12 });
      else if (tab === "opps")     data = await window.ObaraBackend?.spareMatrix?.opportunities?.(customerId);
      else if (tab === "obsolete") data = await window.ObaraBackend?.spareMatrix?.obsolete?.(Number(obsMonths) || 18);
...
window.SparesMatrix = WiredSparesWorksheet;
