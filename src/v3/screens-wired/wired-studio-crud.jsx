...
const WiredStudioCRUD = () => {
  ...
  const customers = useFetch(
    () => window.ObaraBackend?.customers?.list?.() || studioCrudFetch("/api/customers"),
    []
  );
  ...
  const canWrite = window.RBAC?.canDo?.("studio.write") ?? true;
  ...
  const loadVersions = (id) => {
    ...
    setErr(null);
    Promise.resolve(window.ObaraBackend?.profileVersions?.list?.(id)
  ...
  const saveProfile = async () => {
    ...
    try {
      ...
      await (window.ObaraBackend?.customers?.upsert?.(payload)
...
window.ProfileStudio = WiredStudioCRUD;
