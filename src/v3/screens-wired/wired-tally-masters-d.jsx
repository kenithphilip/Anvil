...
const WiredTallyMasters = () => {
  const [active, setActive] = useStateW("stock_item");

  // Fetch each master type once for KPI counts.
  const stock    = useFetch(() => window.ObaraBackend?.tally?.listMasters?.("stock_item")   || Promise.resolve({ masters: [] }), []);
  const ledgers  = useFetch(() => window.ObaraBackend?.tally?.listMasters?.("ledger")       || Promise.resolve({ masters: [] }), []);
  const gst      = useFetch(() => window.ObaraBackend?.tally?.listMasters?.("gst_ledger")   || Promise.resolve({ masters: [] }), []);
  const uoms     = useFetch(() => window.ObaraBackend?.tally?.listMasters?.("uom")          || Promise.resolve({ masters: [] }), []);
  const vouchers = useFetch(() => window.ObaraBackend?.tally?.listMasters?.("voucher_type") || Promise.resolve({ masters: [] }), []);
...
window.TallyMasters = WiredTallyMasters;
