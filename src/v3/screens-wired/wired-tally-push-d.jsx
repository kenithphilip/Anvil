...
const WiredTallyPush = () => {
  const queue   = useFetch(() => window.ObaraBackend?.orders?.list?.({ status: "APPROVED", limit: 200 })          || Promise.resolve({ orders: [] }), []);
  const pushed  = useFetch(() => window.ObaraBackend?.orders?.list?.({ status: "EXPORTED_TO_TALLY", limit: 200 }) || Promise.resolve({ orders: [] }), []);
  ...
  const canPush  = !!(window.RBAC && window.RBAC.canDo && window.RBAC.canDo("tally.push"));
  ...
  const handlePush = async (order) => {
    ...
    setFlash(null);
    try {
      await window.ObaraBackend?.tally?.push?.(tallyPushSnapshot(order));
...
window.TallyPush = WiredTallyPush;
