...
const WiredTallyReconcile = () => {
  const exported = useFetch(() => window.ObaraBackend?.orders?.list?.({ status: "EXPORTED_TO_TALLY", limit: 200 }) || Promise.resolve({ orders: [] }), []);
  ...
  const handleReconcile = async (order) => {
    ...
    try {
      const tallyVoucherId =
        ...
        null;
      await window.ObaraBackend?.tally?.reconcile?.({
        orderId: order.id,
        status: "reconciled",
...
window.TallyReconcile = WiredTallyReconcile;
